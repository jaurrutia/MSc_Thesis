...
The optical properties of a single spherical particle, described by the absorption ($C_\text{abs}$), Scattering ($C_\text{sca}$) and extinction ($C_\text{ext}$) cross section,   can be then calculated by substitution of Eqs. \eqref{eq:EthetaSca} and \eqref{eq:EphiSca} into Eqs. \eqref{eq:Csca} and  \eqref{eq:Cext}, respectively. Thus, the scattering cross section is given by
%
\begin{align}
C_\text{sca} = \int_0^\pi\int_0^{2\pi}\qty(\abs{S_2(\theta)}^2E_0^2\cos^2\varphi + \abs{S_1(\theta)}^2E_0^2\sin^2\varphi)\frac{r^2\sin\theta\dd{\theta}\dd{\varphi}}{k^2r^2E_0^2},
\end{align}
%
